  ...
  const fetchAllData = async (month: string) => {
    ...
    try {
      ...
      const filteredEmployees = fetchedEmployees.filter((e) => {
        ...
        const dojMonthStart = new Date(
          Date.UTC(doj.getUTCFullYear(), doj.getUTCMonth(), 1)
        );
        ...
        if (poEndDate) {
          const poEndMonthEnd = new Date(
            Date.UTC(poEndDate.getUTCFullYear(), poEndDate.getUTCMonth() + 1, 0)
          );
      ...
      <CardHeader>
        <div className="flex justify-between items-center">
          <div>
            ...
            <CardDescription>
              Update the status for each employee for the selected month.
            </CardDescription>
          </div>
